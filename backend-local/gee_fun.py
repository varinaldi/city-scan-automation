...
# NOTE: Earth Engine must be initialized in your notebook with:
# ee.Authenticate()
# ee.Initialize(project='your-project-id')

# ALL EXPORTS GO TO GOOGLE DRIVE (folder: city-scan-outputs)
# You need to manually download files from Google Drive and move to mnt/ folder
...
def gee_forest(city_name_l, aoi_file, cloud_bucket, output_dir):
    ...
    # Export results to Google Drive or Cloud Storage bucket ------------------
    if cloud_bucket is None:
        # Export to Google Drive (for local processing)
        task0 = ee.batch.Export.image.toDrive(**{'image': forestCover23,
                                                'description': f'{city_name_l}_forest_cover23',
                                                'region': AOI,
                                                'scale': 30.92,
                                                'folder': 'city-scan-outputs',
        ...
        print(f"✓ Export tasks started. Files will be saved to Google Drive folder 'city-scan-outputs'")
        print("  - Download them from Google Drive and copy to your mnt/ folder manually")
    else:
