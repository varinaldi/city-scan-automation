...
def _load_clipped_raster(city_name, output_dir, year):
    raster_path = os.path.join(output_dir, "spatial", f"{city_name}_ghs_pop_E{year}.tif")
...
def plot_rastermap(
    ...
    cbar.set_label(f"Population Distribution in {year}", rotation=90)

    ax.set_title(f"{city_name} – Population - {year}")
    ax.axis("off")
    ...
    out_path = os.path.join(img_dir, f"{city_name}_ghs_pop_E{year}_map.png")
...
def plot_histogram(
    ...
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_title(f"{city_name} – {year} Population Distribution Histogram")

    # Save
    img_dir = os.path.join(output_dir, "images")
    os.makedirs(img_dir, exist_ok=True)

    out_path = os.path.join(img_dir, f"{city_name}_ghs_pop_E{year}_histogram.png")
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
