...
def plot_rastermap(
    ...
    """
    Plot solar map using raster clipped to the AOI.
    ...
    # ----------------------------------
    # FIX: Create smooth N-gradation colormap from palette
    # ----------------------------------
    palette = ['#ffebcc', '#f07605']   # low → high
    cmap = LinearSegmentedColormap.from_list(
        "solar_gradient",
        palette,
        N=10   # controls smoothness of gradient
    )
    ...
    cbar.set_label("Power potential of a theoretical 1 kWp PV system (kWh)", rotation=90)

    ax.set_title(f"{city_name} – Solar Photovoltaic Power Potential")
    ax.axis("off")
    ...
    out_path = os.path.join(img_dir, f"{city_name}_solar_map.png")
...
def plot_histogram(
    ...
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_title(f"{city_name} – Solar Potential Distribution Histogram")

    # Save
    img_dir = os.path.join(output_dir, "images")
    os.makedirs(img_dir, exist_ok=True)

    out_path = os.path.join(img_dir, f"{city_name}_solar_histogram.png")
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
