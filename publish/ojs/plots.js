...
function plot_wsf_harmonized(data, {
  ...
  const lines = separate
    ? [
        { data: data.filter(d => d.source === "WSF Evolution"), color: "#2166ac", dash: "6,3", label: "WSF Evolution" },
        { data: data.filter(d => d.source === "WSF Tracker"), color: "#b2182b", dash: "6,3", label: "WSF Tracker" }
      ]
    : [
        { data: data.filter(d => d.source === "WSF Harmonized"), color: "#000000", dash: null, label: "WSF Harmonized" }
      ];
...
function plot_wsf_growth(data, {
  ...
  function rollingAvg(arr, window) {
    ...
    return arr.map((d, i) => {
      ...
      const slice = arr.slice(start, end).filter(v => v.growth_percentage != null && !isNaN(v.growth_percentage));
      if (slice.length === 0) return { ...d, growth_percentage: null };
      const avg = slice.reduce((s, v) => s + v.growth_percentage, 0) / slice.length;
      return { ...d, growth_percentage: Math.round(avg * 100) / 100 };
    });
  }
  ...
  // Apply smoothing
  if (smooth > 1) {
    lines.forEach(l => { l.data = rollingAvg(l.data, smooth); });
  }

  // Filter out null growth values
  lines.forEach(l => { l.data = l.data.filter(d => d.growth_percentage != null && !isNaN(d.growth_percentage)); });

  const allData = lines.flatMap(l => l.data);
  const minYear = d3.min(allData, d => d.year);
  const maxYear = d3.max(allData, d => d.year);
  const maxVal = d3.max(allData, d => d.growth_percentage);
  ...
  const svg = d3.create("svg")
    ...
    .style("background", "white").style("font", "10px system-ui");
  ...
  const yTicks = Math.max(4, Math.min(10, Math.floor(width * 0.6) / 60));
  const yAxisGroup = g.append("g").attr("class", "y-axis");
  yAxisGroup.call(d3.axisLeft(yScale).ticks(yTicks).tickSizeOuter(0).tickSizeInner(-innerWidth));
  yAxisGroup.select(".domain").remove();
  yAxisGroup.selectAll(".tick line").attr("stroke", "#f0f0f0").attr("stroke-width", 1);
  yAxisGroup.selectAll(".tick text").attr("fill", "currentColor").attr("font-size", "10px").attr("font-family", "system-ui").attr("dx", "-0.3em");

  // X-axis
  const xAxisGroup = g.append("g").attr("class", "x-axis").attr("transform", `translate(0,${innerHeight})`);
  const xTicks = Math.max(5, Math.min(15, Math.floor(width / 80)));
  xAxisGroup.call(d3.axisBottom(xScale).ticks(xTicks).tickFormat(d3.format("d")).tickSizeOuter(0).tickSizeInner(-innerHeight));
  xAxisGroup.select(".domain").attr("stroke", "currentColor").attr("stroke-width", 1);
  xAxisGroup.selectAll(".tick line").attr("stroke", "#f0f0f0").attr("stroke-width", 1);
  xAxisGroup.selectAll(".tick text").attr("fill", "currentColor").attr("font-size", "10px").attr("font-family", "system-ui").attr("dy", "1.5em");
  ...
  lines.forEach(l => {
    if (l.data.length === 0) return;
    g.append("path").datum(l.data)
      .attr("fill", "none").attr("stroke", l.color)
      .attr("stroke-width", strokeWidth).attr("stroke-dasharray", l.dash)
      .attr("d", lineGen);
    g.selectAll(null).data(l.data)
      .enter().append("circle")
      .attr("cx", d => xScale(d.year)).attr("cy", d => yScale(d.growth_percentage))
      .attr("r", dotRadius).attr("fill", l.color)
      .style("cursor", "default")
      .on("mouseover", function(event, d) {
        d3.selectAll(".d3-tooltip").remove();
        d3.select("body").append("div").attr("class", "d3-tooltip")
          .style("position", "absolute").style("background", "white").style("color", "black")
          .style("padding", "8px 12px").style("border", "1.8px solid #000000")
          .style("font-family", "system-ui").style("font-size", "12px").style("line-height", "1.4")
          .style("box-shadow", "0 2px 8px rgba(0,0,0,0.15)")
          .style("pointer-events", "none").style("z-index", "1000").style("opacity", 0)
          .html(`<div>${l.label}</div><div>Year: ${d.year}</div><div>Growth: ${d.growth_percentage}%</div>`)
          .style("left", (event.pageX - 60) + "px").style("top", (event.pageY - 60) + "px")
          .transition().duration(200).style("opacity", 1);
      })
      .on("mousemove", function(event) {
        const tooltip = d3.select(".d3-tooltip");
        if (!tooltip.empty()) tooltip.style("left", (event.pageX - 60) + "px").style("top", (event.pageY - 60) + "px");
      })
      .on("mouseout", function() { d3.selectAll(".d3-tooltip").transition().duration(200).style("opacity", 0).remove(); });
  });
  ...
  const legendGroup = svg.append("g").attr("class", "legend");
  lines.forEach((l, i) => {
    const legendItem = legendGroup.append("g").attr("transform", `translate(${legendX}, ${legendY + i * legendSpacing})`);
    legendItem.append("line").attr("x1", 0).attr("x2", 20).attr("y1", 0).attr("y2", 0)
      .attr("stroke", l.color).attr("stroke-width", 2).attr("stroke-dasharray", l.dash);
    legendItem.append("text").attr("x", 25).attr("y", 0).attr("dy", "0.32em")
      .attr("font-size", "12px").attr("font-family", "system-ui").attr("fill", "currentColor")
      .text(l.label + (smooth > 1 ? ` (${smooth}-yr avg)` : ""));
  });

  // Title
  const titleSuffix = smooth > 1 ? ` (${smooth}-year average)` : "";
  svg.append("text").attr("x", 20).attr("y", 25)
    .attr("text-anchor", "start").attr("fill", "currentColor")
    .attr("font-size", "18px").attr("font-family", "system-ui")
    .text(`Urban Growth Rate, ${minYear}-${maxYear}${titleSuffix}`);
  svg.append("text").attr("x", 20).attr("y", 45)
    .attr("text-anchor", "start").attr("fill", "currentColor")
    .attr("font-size", "16px").attr("font-family", "system-ui").attr("font-style", "italic")
    .text(cityName);
...
function plot_flood_prob(data, {
  ...
  const bins = [
    { key: "1-in-1,000 year", label: "1-in-1,000 year", color: "#A7BCEE", dash: null },
    { key: "1-in-100 year", label: "1-in-100 year", color: "#6478D9", dash: "4,3" },
    { key: "1-in-10 year", label: "1-in-10 year", color: "#1B35C4", dash: "2,2" }
  ];
  ...
  const maxVal = d3.max(data, d => Math.max(...bins.map(b => d[b.key] || 0)));
