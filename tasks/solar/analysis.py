...
def plot_solar_seasonality(
    ...
    for band_idx in range(clipped_image.shape[0]):
        ...
        monthly_stats.append({
            "month": band_idx + 1,
            "max": float(valid_data.max()) if valid_data.size > 0 else np.nan,
            "min": float(valid_data.min()) if valid_data.size > 0 else np.nan,
            "mean": float(valid_data.mean()) if valid_data.size > 0 else np.nan,
        })

    df = pd.DataFrame(monthly_stats).sort_values("month").reset_index(drop=True)
...
def compute_histogram(city_name: str, output_dir: str) -> Optional[pd.DataFrame]:
    """
    Compute PV potential area histogram (from clean_pv_area in clean.py).
    Bins: <3.5 (Less than Favorable), 3.5-4.5 (Favorable), >4.5 (Excellent)
    Output: {city}_pv_area.csv with columns: bin, condition, count, percentage
    """
    spatial_dir = os.path.join(output_dir, "spatial")
    raster_path = os.path.join(spatial_dir, f"{city_name}_solar.tif")
    output_path = os.path.join(output_dir, "tabular", f"{city_name}_pv_area.csv")

    if not os.path.exists(raster_path):
        logger.error(f"Solar raster not found: {raster_path}")
        return None
    ...
    logger.info(f"PV area histogram saved to: {output_path}")
