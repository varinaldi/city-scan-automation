...
def datacollection(
    ...
    GHS builtup rasters for an AOI.
    ...
    # ----------------------------
    logger.info("Downloading GHSL tile shapefile...")
    ...
    gcs_success = False
    ghsl_url = "https://storage.googleapis.com/city-scan-global-public/ghsl/GHSL2_0_MWD_L1_tile_schema_land.shp"
    if ghsl_url:
        logger.info(f"Attempting to download GHSL tile shapefile from GCS: {ghsl_url}")
        try:
            tile_gdf = gpd.read_file(ghsl_url).to_crs(mollweide_proj)
            gcs_success = True
            logger.info("Loaded GHSL tile shapefile from GCS.")
        except Exception as e:
            logger.info(f"GCS download failed: {e}")
            gcs_success = False

    # --- If GCS fails, fall back to GHSL website (ZIP) ---
    if not gcs_success:
        tile_shp_link = "https://ghsl.jrc.ec.europa.eu/download/GHSL_data_54009_shapefile.zip"
        logger.info("Falling back to GHSL website...")
        ...
        try:
            ...
            with open(tmp_zip_path, "wb") as f:
                f.write(r.content)
            logger.info(" Downloaded GHSL shapefile ZIP from website.")
            ...
            with zipfile.ZipFile(tmp_zip_path, "r") as z:
                ...
                logger.info(f"Found shapefile inside ZIP: {shp_name}")
    ...
    logger.info(" GHSL tile shapefile successfully prepared.")


    # ----------------------------
    # STEP 2: Find intersecting tiles (use actual AOI geometry NOT bbox)
    # ----------------------------
    logger.info("Finding intersecting tiles...")
    ...
    if intersecting_tiles.empty:
        logger.info("No intersecting GHSL tiles found for AOI. Exiting.")
        os.remove(tmp_zip_path)
        return

    tile_ids = intersecting_tiles[tile_id_col].tolist()
    logger.info(f"Found {len(tile_ids)} intersecting tiles: {tile_ids}")
    ...
    for year in tqdm(years, desc="Years"):
        ...
        for tile in tile_ids:
            ...
            try:
                ...
                logger.info(f"Tile not available or failed to download: {url} -> {e}")
                continue
            ...
            with zipfile.ZipFile(tile_zip_path, "r") as z:
                ...
                if not tif_members:
                    logger.info(f"⚠️ No .tif found in {tile_zip_path}")
                    continue
                for member in tif_members:
                    try:
                        ...
                        if len(tif_data) < 1_000_000:
                            logger.info(f"⚠️ Skipping tiny or invalid TIFF: {member} ({len(tif_data)} bytes)")
                            continue
                        ...
                        logger.info(f"⚠️ Failed to read TIFF {member}: {e}")
                        continue

        if not tifs:
            logger.info(f"No tiles downloaded for year {year} (skipping).")
        ...
        try:
            ...
            logger.info(f"⚠️ Merge failed, trying with valid-only datasets: {e}")
        ...
        out_tif = os.path.join(spatial_dir, f"{city_name}_ghs_built_E{year}.tif")
        ...
        logger.info(f"Saved clipped+reprojected GeoTIFF: {out_tif}")
    ...
    logger.info("ghs_builtup finished successfully.")
