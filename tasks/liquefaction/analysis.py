...
# From Caroline's clean.py — clean_l_area()
def compute_histogram(
    ...
    """
    Bin liquefaction susceptibility into categorical levels for visualization.
    Produces l_area.csv with columns: bin, susceptibility, count, percentage.
    """

    logger.info("Starting liquefaction histogram analysis…")
    ...
    if analysis_data.size == 0:
        logger.error("No valid liquefaction values.")
        return None
    ...
    tabular_dir = os.path.join(output_dir, "tabular")
    os.makedirs(tabular_dir, exist_ok=True)
    output_path = os.path.join(tabular_dir, f"{city_name}_l_area.csv")

    try:
        result_df.to_csv(output_path, index=False)
        logger.info(f"Liquefaction histogram saved to: {output_path}")
    except Exception as e:
